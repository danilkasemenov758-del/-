...
const state = {
  ...
  saved: JSON.parse(localStorage.getItem("savedForTrip") || "[]"),
  activeOrderId: 1,
  filter: "all",
  toast: "",
...
async function loadRemoteData() {
  ...
  try {
    ...
    state.toast = "База недоступна, работаем локально";
    render();
    clearToastLater();
  }
}
...
function queueAction(type, payload) {
  ...
  state.toast = type === "take-kit" ? "Комплект добавлен в очередь" : "Реквизит добавлен в очередь";
  saveState();
  render();
  sendAction(action);
  clearToastLater();
...
function syncPill() {
  const text = state.syncQueue.length ? `к отправке: ${state.syncQueue.length}` : "все синхронизировано";
...
function deniedScreen() {
  return appFrame(`
    ...
    <div class="footer-brand">Точка праздника<span>проект Банный Бон</span></div>
  `);
}
...
function ordersScreen() {
  return appFrame(`
    ...
    <div class="content-stack">
      ...
      <div class="chips">
        <button class="chip active">Мои</button>
        <button class="chip">Неделя</button>
  ...
  return appFrame(`
    ...
    <div class="content-stack">
      <section class="panel">
        ...
        <input class="booking-input" data-booking="email" placeholder="E-mail" value="${state.booking.email}" style="margin-top: 8px" />
      ...
      <section class="panel">
        ...
        <div class="booking-grid">
          ...
          <label>
            ...
            <input class="booking-input" type="number" min="1" max="8" step="1" data-booking="duration" value="${state.booking.duration}" />
          </label>
        </div>
      </section>
      ...
      ${
        state.user.role === "actor"
          ? `<section class="panel">
              ...
              <p class="small-text">${accepted ? `Заказ уже принял: ${accepted.name}` : "Можно принять заказ. Если нет сети, отметка сохранится и отправится позже."}</p>
              <button class="primary-button" style="margin-top: 12px" data-action="accept-order" data-order-id="${order.id}" ${accepted ? "disabled" : ""}>
                ${accepted ? "Заказ принят" : "Принять заказ"}
              </button>
            </section>`
          : `<section class="panel">
    ...
    <div class="content-stack">
      ...
      <div class="orders-stack">
        ${list
          .map(
            (item) => `
              <button class="prop-row" data-action="take-prop" data-prop-id="${item.id}">
                <span><strong>${item.name}</strong><span>${statusText(item.status)} · ${item.place}</span></span>
                <span class="row-icon ${item.status === "mine" ? "done" : ""}" aria-label="${item.status === "mine" ? "У вас" : "Взять"}">
                  ${item.status === "mine" ? "✓" : "+"}
                </span>
              </button>
            `
  ...
  return appFrame(`
    ...
    <div class="content-stack">
      ...
      ${programs
        .map(
          (program) => `
            <button class="order-row" data-route="program-detail">
...
function profileScreen() {
  ...
  return appFrame(`
    ...
    <div class="content-stack">
      ...
      <button class="primary-button" data-action="mock-sync">Обновить данные</button>
...
document.addEventListener("click", (event) => {
  ...
  if (routeButton) {
    const orderId = routeButton.dataset.orderId;
    setRoute(routeButton.dataset.route, orderId ? { activeOrderId: Number(orderId) } : {});
    return;
  }
  ...
  if (action === "create-order") {
    queueAction("create-order", { ...state.booking, actorId: state.user.id });
    state.toast = "Заказ добавлен";
    setRoute("orders");
    clearToastLater();
  }

  if (action === "contact-admin" || action === "report") {
    tg?.showAlert?.("Сообщение администратору добавим при подключении бота.");
    if (!tg) alert("Сообщение администратору добавим при подключении бота.");
  }
});
