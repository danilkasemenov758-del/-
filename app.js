                ...
                .map(
                  (order) => `
                    <div class="managed-row">
                      ...
                      ${
                        state.user.role === "admin"
                          ? `<button class="delete-row-button" data-action="delete-order" data-order-id="${order.id}">Удалить</button>`
                          : ""
                      }
                    </div>
                  `
...
document.addEventListener("click", (event) => {
  ...
  if (action === "send-report") {
    queueAction("report", { text: state.reportText, actorId: state.user.id, route: state.route });
    state.reportText = "";
    setRoute("profile");
  }
});
